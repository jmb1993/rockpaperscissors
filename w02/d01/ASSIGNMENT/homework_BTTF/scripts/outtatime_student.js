...
function displayMcFlys() {
  ...
  for (var i = 0; i < family.length; i++) {
    var familyNamesList = document.createElement('li');

    var familyName = family[i].fname + ' ' + family[i].lname;
    familyNamesList.innerHTML = (familyName);

    familyNamesList.className = 'family';
    familyList.appendChild(familyNamesList);
  }
  // Give each McFly a class of 'family'

  // Give Marty an id of 'marty';
  familyList.childNodes[0].id = 'marty';
};
...
marty = document.createElement('li');
marty.textContent = 'Marty McFly';
marty.id = 'marty';
twinPines = document.getElementById('Twin Pines Mall');
doc = document.createElement('li');
doc.id = 'doc';
einstein = document.createElement('li');
doc.textContent = friends[0];
einstein.textContent = friends[1];
twinPines.appendChild(marty);
twinPines.appendChild(doc);
twinPines.appendChild(einstein);
//sorry Marty, you don't get to go to the mall because apparently you can't append on your node...OR you do and you just happen to be in two places at once.
...
var libyans = document.createElement('li');
libyans.textContent = enemies[4];
twinPines.appendChild(libyans);
  ...
  document.getElementById('doc').style.textDecoration = 'overline';
...
function goingBackInTime(){
  ...
  var hillHS = document.getElementById('Hill Valley HighSchool');
  var marty = document.getElementById('marty');
  hillHS.appendChild(marty); //idk how else you want to append Marty so you just aren't gonna go back to high school but at least you work with the debugger!



 // Find an element with id `nineteen-fifty-five`.
 var nFF = document.getElementById('nineteen-fifty-five');
 // Find an element with id `nineteen-eighty-five`.
 var nEF = document.getElementById('nineteen-eighty-five');
 // Change the element with the id `nineteen-eighty-five` so that it now has the class `last-time-departed`.
 nEF.className = 'last-time-departed';
 // Change the element with the id `nineteen-fifty-five` so that is now has the class `present-time`.
 nFF.className = 'present-time';


// Set the `Twin Pines Mall` element to display none
document.getElementById('Twin Pines Mall');
...
//cool because they were never there.


// Find Marty and put him in the `Peabody Farm`.
//marty can't ever append so i can't find him he's lost in the space time continuum or something.
...
var marty = document.getElementById('marty');
marty.setAttribute('aka', 'Calvin');
// Also Marty's mom, Lorraine isn't a McFly yet.  Change her last name to 'Baines'.
//cant get it to work!
//document.getElementsByClassName('familyList')[2].innerHTML = 'Lorraine Baines';
...
var enchant = document.createElement('div')
enchant.id = 'enchantment-dance';
var school = document.getElementById('Hill Valley HighSchool');
school.appendChild(enchant);
...
//not working  school.appendChild(enemy[i])
}
var libyans = document.createElement('li');
libyans.textContent = enemies[4];
twinPines.appendChild(libyans);
...
// Meanwhile, take the line-through text decoration off of Doc's name and put him at the Clock Tower along with his dog(circa 1955), Copernicus.
  ...
  document.getElementById('doc').style.textDecoration = 'none';
